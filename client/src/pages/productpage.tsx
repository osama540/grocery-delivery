...
const ProductPage = () =>{
  ...
  const currency = import.meta.env.VITE_CURRENCY_SYMBIL || "$";
  const { id } = useParams()
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <button onClick={()=> navigate(-1)} className="mb-6 flex items-center gap-1.5 text-sm text-app-text-light hover:text-app-green transiton-colors">
                ...
                <div className="p-6 md:p-10 flex flex-col justify-center">
                  ...
                  {product.rating > 0 && (
                    <div className="flex items-center gap-2 mb-5">
                      <div className="flex items-center gap-0.5">
                        {[1, 2, 3, 4, 5].map((stars)=>(
                          <StarIcon key={stars} className={`w-4 h-4 ${stars <= Math.round(product.rating) ? 
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    className={`flex-1 py-3 font-semibold rounded-xl transition-colors flex-center gap-2
                      disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98] ${inCart ? "bg-app-cream text-app-green border border-app-green" : 
