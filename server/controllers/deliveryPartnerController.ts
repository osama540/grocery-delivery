...
export const getDeliveryDelail = async (req: Request, res: Response) => {
...
export const completeDelivery = async (req: Request, res: Response) => {
    ...
    if(order.deliveryOtp !== opt){
        return res.status(500).json({message: "Invalid OPT"})
    }
...
// Cancel delivery
// PUT /api/delivery/my-delivries/:id/cancel
...
// Update order status
// PUT /api/delivery/my-deliveries/:id/ststus
export const updateDeliveryStatus = async (req: Request, res: Response) => {
    const { status } = req.body;
    const allowedStatuses = ["Packed", "Out foe Delivery"];
...
export const updateLocation = async (req: Request, res: Response) => {
    ...
    const order = await prisma.order.findFirst({
        where: {
            ...
            status: {in: ['Asigned', "Packed", "Out for Delivery"]}
        }
    })
